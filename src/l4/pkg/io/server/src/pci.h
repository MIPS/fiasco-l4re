...
namespace Hw { namespace Pci {
  ...
  public:
    ...
    explicit Bus(unsigned char bus) : num(bus), subordinate(bus), irq_router(0) {}
  ...
  public:
    ...
    l4_uint8_t next() { l4_uint8_t r; read(1, &r); return r; }
  };
  ...
  private:
    typedef cxx::H_list<Saved_cap> Cap_list;
    union
    {
      l4_uint32_t w[16];
      l4_uint16_t h[16*2];
  ...
  public:
    explicit Root_bridge(unsigned bus_nr, Hw::Device *host)
    : Bus(bus_nr), _host(host)
    {}
...
public:
  ...
  l4_uint32_t recheck_bars(unsigned decoders);
  bool enable_rom();
  ...
  bool supports_msi() const { return flags.msi(); }
  ...
  l4_uint32_t vendor_device_ids() const;
  l4_uint32_t class_rev() const;
  l4_uint32_t subsys_vendor_ids() const;
  ...
  void pm_save_state(Hw::Device *);
  void pm_restore_state(Hw::Device *);
...
public:
  ...
  explicit Pci_pci_bridge_basic(Hw::Device *host, Bus *bus)
  : Bus(0), Dev(host, bus), pri(0)
  {}
...
{
  explicit Port_root_bridge(unsigned bus_nr, Hw::Device *host)
  : Root_bridge(bus_nr, host) {}
...
int register_root_bridge(int segment, Root_bridge *b);
