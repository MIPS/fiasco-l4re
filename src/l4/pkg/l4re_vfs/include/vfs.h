...
public:
   ...
   *
   * \retval buf This buffer is filled with the status information.
   * \return 0 on success, or <0 on error.
   */
   ...
   * command).
   *
   * \return flags such as #O_RDONLY, #O_WRONLY, #O_RDWR, #O_DIRECT, #O_ASYNC,
   *         #O_NOATIME, #O_NONBLOCK, or <0 on error.
   */
...
public:
   ...
   * \return 0 on success, or <0 on error.
   */
  virtual int rmdir(const char *) throw() = 0;
...
public:
  ...
  void add_child_node(Mount_tree *cld);
...
inline
void
Mount_tree::add_child_node(Mount_tree *cld)
{
  cld->_sib = _cld;
  _cld = cld;
}
