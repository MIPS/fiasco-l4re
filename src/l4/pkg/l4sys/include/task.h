 ...
 * \brief Class definition of the Task kernel object.
 *
 * <c>\#include <l4/sys/task.h></c>
 *
 * The L4 task class represents a combination of the address spaces provided
 * by the L4 micro kernel. A task consists of at least a memory address space
 * and an object address space. On IA32 there is also an IO-port address space.
 *
 * A task object can be created using a Factory, see \ref l4_factory_api
 * (l4_factory_create_task()).
 */

/**
 * \brief Map resources available in the source task to a destination task.
 * \ingroup l4_task_api
 *
...
/**
 * \brief Revoke rights from a task.
 * \ingroup l4_task_api
 *
 * \param task          Capability selector of destination task
...
/**
 * \brief Test whether a capability has child mappings (in another task).
 * \ingroup l4_task_api
 *
 * \param task         Capability selector of the destination task to do the
 *                     lookup in
