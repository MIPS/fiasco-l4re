...
Cap::find_dynamic_type(lua_State *l) const
{
  ...
  get_method_table(l, name.value<char const *>());
  if (lua_isnil(l, -1))
...
{
  if (lua_gettop(l) > 2)
    luaL_error(l, "too man arguments to L4.cast (expected 2:type and cap)");
...
init_lua_cap(lua_State *l)
{
  ...
  luaL_register(l, package, _cast_f);
  lua_newtable(l);
  lua_setfield(l, -2, CAST_TABLE);
