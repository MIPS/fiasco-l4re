...
class Acpi_sci : public Kernel_irq_pin, public L4::Server_object
{
public:
...
int Acpi_sci::dispatch(l4_umword_t, L4::Ipc::Iostream &)
{
  if (!_isr(_context))
    ...
    void                      *context)
{
  ...
  L4::Cap<L4::Irq> irq = registry->register_irq_obj(sci);
  if (!irq.is_valid())
    {
