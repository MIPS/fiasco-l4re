...
static void find_memory()
{
  ...
  pages = (typeof(pages))Single_page_alloc_base::_alloc(sizeof(*pages) * total_pages);
...
init_kip_ds()
{
  kip_ds = new Moe::Dataspace_static(const_cast<l4_kernel_info_t*>(kip()), L4_PAGESIZE, 0);
  if (!kip_ds)
    {
...
static void GC_CALLBACK finalization_notifier()
{
  l4_msg_regs_t b;
  l4_msg_regs_t *mr = l4_utcb_mr();
  // GC_dump();
  memcpy(&b, mr->mr, sizeof(l4_msg_regs_t));
  GC_invoke_finalizers();
  memcpy(mr->mr, &b, sizeof(l4_msg_regs_t));
}
...
int main(int argc, char**argv)
{
  ...
  info.printf("Hello world\n");
    ...
    {
      using L4Re::Env;
      Env::env()->scheduler()->run_thread(L4::Cap<L4::Thread>(L4_BASE_THREAD_CAP), l4_sched_param(0xff));
    }
  ...
  for (unsigned long i = (unsigned long)_stext; i < (unsigned long)_etext; i+= L4_PAGESIZE)
    {
      l4_task_unmap(l4_utcb(), L4_BASE_TASK_CAP, l4_fpage(i, L4_PAGESHIFT, L4_FPAGE_W), L4_FP_ALL_SPACES);
    }
#endif
  ...
  try
    {
      ...
      GC_set_finalizer_notifier(finalization_notifier);
      ...
      root_name_space()->register_obj("log", Obj(Obj::F_rw, L4_BASE_LOG_CAP));
