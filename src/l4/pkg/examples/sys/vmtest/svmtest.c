...
static void handle_vmexit(void)
{
  ...
  ++exitcnt;
  printf("iteration=%d, exit code=%llx", exitcnt, vmcb_s->control_area.exitcode);
...
static void run_test(int np_available)
{
  ...
  if (l4_error(tag))
    {
      printf("Failed to create new task: %ld\n", l4_error(tag));
      exit(1);
    }
...
int main(void)
{
  ...
  if (check_svm())
    {
      printf("No SVM capable CPU. Bye.\n");
      return 1;
    }

  timer_irq = l4re_util_cap_alloc();
  if (l4_is_invalid_cap(timer_irq))
    {
      printf("failed to get cap\n");
      return 1;
    }
