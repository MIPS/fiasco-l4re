/*
 * Derived in part from linux arch/mips/include/asm/mips-cpc.h
 *
...
#include "io.h"
#define __iomem
typedef Address phys_t;
typedef Unsigned32 u32;
#define __raw_readl(a) Io::read<unsigned long>((unsigned long)(a))
#define __raw_writel(val,a) Io::write<unsigned long>((val), (unsigned long)(a))
...
{								\
	return (u32 *)((char*)mips_cpc_base + (off));			\
}								\
								\
static inline u32 read_cpc_##name(void)				\
{								\
	return __raw_readl((char*)mips_cpc_base + (off));		\
}

#define BUILD_CPC__W(name, off) \
static inline void write_cpc_##name(u32 value)			\
{								\
	__raw_writel(value, (char*)mips_cpc_base + (off));		\
}
