...
#ifdef CIRRUSFB_DEBUG
#ifdef L4_DRIVERS
	cirrusfb_dbg_reg_dump(info, (caddr_t)regbase);
#else
...
#ifdef L4_DRIVERS
	err = fb_find_mode(&info->var, info, mode_option, NULL, 0, NULL, L4_CIRRUSFB_DEFAULT_BPP);
#else
...
static struct fb_info _fb_info_struct;
static struct cirrusfb_info _cinfo_struct;
...
static l4_addr_t request_vga_ports(void)
{
  ...
  if (vaddr == 0)
    {
      dev_err(void, "Could not map VGA config ports\n");
      return 0;
    }
...
static l4_addr_t request_video_region(void)
{
  l4io_device_handle_t fbdev_hdl;
  l4io_resource_handle_t fbres_hdl;
  l4_addr_t vaddr = 0;

  if (l4io_lookup_device("PCI/CC_03", &fbdev_hdl, 0, &fbres_hdl))
    {
      dev_err(void, "Could not find VGA video memory\n");
      return 0;
    }
  ...
  if (vaddr == 0)
    {
      dev_err(void, "Could not map VGA video memory\n");
      return 0;
    }
...
static int
cirrusfb_vbus_pci_register(unsigned int bus, unsigned int devfn, 
				 const struct pci_device_id *ent)
{
  int ret;
  struct cirrusfb_info *cinfo;
  unsigned long board_addr, board_size;
  l4_size_t size0;
  l4_addr_t malta_cirrusfb_ctrl_base_virt;
  l4_addr_t malta_cirrusfb_video_base_virt;

  // only using resource 0 (video base), ignore mmio resource 1
  vbus_pci_resource(bus, devfn, 0, &board_addr, &size0);
  if (!board_addr)
    return -L4_ENODEV;

  vbus_pci_enable_memory(bus, devfn);

  dev_info(info->device, "Found PCI cirrusfb boardtype %ld\n", ent->driver_data);
  dev_info(info->device, "Board address: 0x%lx-%lx\n", board_addr, board_addr + size0 - 1);

  malta_cirrusfb_ctrl_base_virt = request_vga_ports();
  malta_cirrusfb_video_base_virt = request_video_region();

  if ((malta_cirrusfb_ctrl_base_virt == 0) ||
      (malta_cirrusfb_video_base_virt == 0)) {
    return 1;
  }

  dev_info(info->device, "cirrusfb control register base is at virtual %p\n",
         (void*)malta_cirrusfb_ctrl_base_virt);
  dev_info(info->device, "cirrusfb video buffer is at virtual %p\n",
         (void*)malta_cirrusfb_video_base_virt);

  // Using static init, cinit for now
  info = &_fb_info_struct;
  info->par = &_cinfo_struct;
  cinfo = info->par;

  cinfo->btype = (enum cirrus_board) ent->driver_data;
  cinfo->regbase = (u8 __iomem *)malta_cirrusfb_ctrl_base_virt;

  board_size = cirrusfb_get_memsize(info, (u8 __iomem *)cinfo->regbase);
  info->fix.smem_start = board_addr;
  info->screen_base = (char __iomem *)malta_cirrusfb_video_base_virt;
  info->screen_size = board_size;
...
static struct vbus_pci_driver cirrusfb_vbus_pci_driver = {
	.name		= "cirrusfb",
	.id_table	= cirrusfb_pci_table,
	.probe		= cirrusfb_vbus_pci_register,
	//.remove		= NULL,
};
#endif /* L4_DRIVERS */
...
{
	cirrusfb_setup(options);
        return 0;
}

int l4_cirrusfb_init(void)
{
	return vbus_pci_register(&cirrusfb_vbus_pci_driver);
}
