...
App_model::open_file(char const *name)
{
  ...
  f = chkcap(ens.query<L4Re::Dataspace>(name), name, 0);
...
App_model::copy_ds(Dataspace dst, unsigned long dst_offs,
                   ...
                   unsigned long size)
{
  if (dst->copy_in(dst_offs, src.get(), src_offs, size) < 0)
    {
      L4::Cap<L4Re::Rm> r = L4Re::Env::env()->rm();
      L4Re::Rm::Auto_region<char const *> data;
      chksys(r->attach(&data, size, L4Re::Rm::Search_addr | L4Re::Rm::Read_only,
	    src.get(), src_offs));

      // FIXME: buggy:
      //memcpy(paddr + page_offs, data.get() + page_offs, fsz);
    }
}
